...
class Assigned extends Component {
    ...
    rows = (s,v) => {
        return (
            ...
            >
				<WorkListADMIN key={v} data={s}/>
            </Link>
        )
    };

    render(){
		const {refreshing,dataSource,hasMore,rollingState,condition} = this.state;
        return (
            <div style={{
                padding:'0 .3rem'
            }}>
...
Assigned = connect(state => {return {
	state:state.AdminWorksheetList.assigned
}})(Assigned);
